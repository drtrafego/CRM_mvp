...
export function Board({ columns: initialColumns, initialLeads, onLeadsChange, orgId }: BoardProps) {
  ...
  function onDragEnd(event: DragEndEvent) {
    ...
    if (active.data.current?.type === "Column") {
      if (activeId !== overId) {
        setColumns((columns) => {
          ...
          updateColumnOrder(newOrder.map(c => c.id), orgId)
            .then((response) => {
    ...
    if (active.data.current?.type === "Lead") {
      setLeads((leads) => {
        ...
        updateLeadStatus(movedLead.id, movedLead.columnId!, newPosition, orgId)
          .catch(err => {
  ...
  async function handleCreateColumn() {
    ...
    await createColumn(newColumnName, orgId);
    setNewColumnName("");
        ...
        <DragOverlay>
          {activeColumn && (
            <div className="opacity-80 rotate-2 cursor-grabbing">
              <Column column={activeColumn} leads={getLeadsByColumn(activeColumn.id)} orgId={orgId} />
            </div>
          )}
          {activeLead && (
            <div className="opacity-80 rotate-2 cursor-grabbing">
